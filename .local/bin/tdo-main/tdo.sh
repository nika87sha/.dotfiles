...
Commands:
-b | --book   | b | book    book list
-y | --yearly | b | yearly  List year
...
book() {
    root="$NOTES_DIR"
    book_file="$root/$1.md"
    template="$root/templates/Book.md"
    if [ ! -f "$book_file" ]; then
        create_file "$book_file" "$template"
        $filename_as_title && echo -e "# $1" >>"$book_file"
        $add_new_note_timestamp && add_timestamp "$book_file" "$note_timestamp_format"
    fi
    write_file "$book_file" "$root"
}

yearly() {
    root="$NOTES_DIR"
    yearly_file="$root/$1.md"
    template="$root/templates/Yearly.md"
    if [ ! -f "$yearly_file" ]; then
        create_file "$yearly_file" "$template"
        $filename_as_title && echo -e "# $1" >>"$yearly_file"
        $add_new_note_timestamp && add_timestamp "$yearly_file" "$note_timestamp_format"
    fi
    write_file "$yearly_file" "$root"
}
...
main() {
    ...
    case "$1" in
    -b | --bppl | b | book) book "$2" ;;
    -y | --yearly | y | yearly) yearly "$2";;
